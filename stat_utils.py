...
def get_univariate_dist(data, kernel="gau", fft=True, bw="scott", gridsize=100, cut=3, clip=None):
...
def interpolate_densities(grid_1, density_1, grid_2, density_2):
    def create_new_grid(grids):
        ...
        higher_end_boundary = grids[0][-1] if grids[0][-1] > grids[1][-1] else grids[1][-1]
